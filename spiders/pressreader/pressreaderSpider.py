...
from spiders.pressreader.author_clean import AuthorClean
            ...
            for y in range(x, len(messages)):
                ...
                if messages[x]["title"] == messages[y]["title"]:
                    if messages[x]["mainBody"][0] == messages[y]["mainBody"][0]:
                        ...
                        if (messages[y]["channel"] not in messages[x]["channel"]) and (
                                messages[x]["channel"] not in messages[y]["channel"]):
    ...
    def supplement(self, baseurl):
        ...
        for x in range(200):
            ...
            if (ret["success"] and ret["result"]):
                ...
                print("采集成功:" + self.displayname + "-发行日期（" + str(publishedtime).split(" ")[0] + "),文章数量（" + str(
                    len(message)) + "）")
                publishedtime += oneday
